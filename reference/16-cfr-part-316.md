# 16 CFR Part 316 — CAN-SPAM Rule (verbatim)

**Source:** Cornell Law School Legal Information Institute, `law.cornell.edu/cfr/text/16/part-316` (mirroring the official Code of Federal Regulations, Title 16, Part 316). Retrieved 2026-09-04, one HTTP GET per section.

**Copyright status:** This is a US federal regulation — a work of the United States Government. 17 U.S.C. §105 bars copyright protection for such works; the text below is public domain. No permission required, no license fee. (Cornell LII's own site chrome/formatting is theirs; the regulatory text itself is not copyrightable by anyone.)

**Why this file exists:** `can-spam-auditor` cites this text directly in every `AUTOMATED` finding. Open a finding in the audit report, `Ctrl-F` the cited section here, confirm the words match. That's the whole verification model — see `rule-map.md` for which finding cites which line.
...
*End of Part 316. `can-spam-auditor`'s `rules.md` cites specific sections above by section number and exact quoted phrase — every such quote is checked by `audit.py`'s own self-test against this file, not just narrated. See `rule-map.md`.*
